...
class Bottle2neck(nn.Module):
    def __init__(self, c1, c2, r1, r2, r3, r4, r5, shortcut=True, g=1, e=0.5):
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c_, c2, 3, 1, g=g)
        ...
        self.add = shortcut and c1 == c2
        # 根据参数r1-r5分配通道
        c1_split = c1 * torch.Tensor([r1, r2, r3, r4, r5])
        c1_split = c1_split.int()

        # 对应的卷积操作
        self.cv3.add_module('conv1x1', nn.Conv2d(c1, c1, 1, 1))
        self.cv4.add_module('conv3x3', nn.Conv2d(c1, c1, 3, 1, 1))
        self.cv5.add_module('conv5x5', nn.Conv2d(c1, c1, 5, 1, 2))
        self.cv6.add_module('conv7x7', nn.Conv2d(c1, c1, 7, 1, 3))
        self.cv7.add_module('conv9x9', nn.Conv2d(c1, c1, 9, 1, 4))
        input_channels = c1_split[0] + c1_split[1] + c1_split[2] + c1_split[3] + c1_split[4]
        self.cv9 = nn.Conv2d(c1, c1_split[0], 1, 1)
        self.cv10 = nn.Conv2d(c1, c1_split[1], 1, 1)
        self.cv11 = nn.Conv2d(c1, c1_split[2], 1, 1)
        self.cv12 = nn.Conv2d(c1, c1_split[3], 1, 1)
        self.cv13 = nn.Conv2d(c1, c1_split[4], 1, 1)
        self.cv14 = nn.Conv2d(input_channels, c2, 1, 1)
        self.add = shortcut and c1 == c2
    ...
    def forward(self, x):
        ...
        out = torch.cat([x8, x9, x10, x11, x12], dim=1)
        out = self.cv14(out)
        return x + out + x7 if self.add else x7
        #return torch.cat([x3, x4, x5, x6, x7], dim=1) if self.add else self.cv2(self.cv1(x))
...
class C3new(nn.Module):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2, r1, r2, r3, r4, r5, n=1, shortcut=True, g=1, e=0.5):
        """Initializes C3 module with options for channel count, bottleneck repetition, shortcut usage, group
        convolutions, and expansion.
        """
        super().__init__()
        ...
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.Sequential(*(Bottle2neck(c_, c_, r1, r2, r3, r4, r5, shortcut=True, g=1, e=1.0) for _ in range(n)))
