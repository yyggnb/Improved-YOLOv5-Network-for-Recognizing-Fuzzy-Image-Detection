...
class Bottle2neck(nn.Module):
    def __init__(self, c1, c2, r1, r2, r3, r4, shortcut=True, g=1, e=0.5):
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c_, c2, 3, 1, g=g)
        ...
        self.add = shortcut and c1 == c2
        # 根据参数r1-r5分配通道
        c1_split = c1 * torch.Tensor([r1, r2, r3, r4])
        c1_split = torch.ceil(c1_split).int()
        ...
        input_channels = c1_split[0] + c1_split[1] + c1_split[2] + c1_split[3]
        self.cv30 = nn.Conv2d(c1, c1_split[0], 1, 1)
        self.cv31 = nn.Conv2d(c1, c1_split[1], 1, 1)
        self.cv32 = nn.Conv2d(c1, c1_split[2], 1, 1)
        self.cv33 = nn.Conv2d(c1, c1_split[3], 1, 1)

        self.add = shortcut and c1 == c2
        self.se = SE(input_channels, input_channels)
        self.cv40 = nn.Conv2d(input_channels, c2, 1, 1)
    def forward(self, x):
        x3 = self.cv3(x)
        x4 = self.cv4(x)
        x5 = self.cv5(x)
        ...
        out = torch.cat([x8, x9, x10, x11], dim=1)
        out = self.se(out)
        out = self.cv40(out)
...
class C3new1(nn.Module):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2, r1, r2, r3, r4, n=1, shortcut=True, g=1, e=0.5):
        """Initializes C3 module with options for channel count, bottleneck repetition, shortcut usage, group
        convolutions, and expansion.
        """
        super().__init__()
        ...
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.Sequential(*(Bottle2neck(c_, c_, r1, r2, r3, r4, shortcut=True, g=1, e=1.0) for _ in range(n)))

    def forward(self, x):
        """Performs forward propagation using concatenated outputs from two convolutions and a Bottleneck sequence."""
        return self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), 1))
class C3new2(nn.Module):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2, r1, r2, r3, r4, n=1, shortcut=True, g=1, e=0.5):
        """Initializes C3 module with options for channel count, bottleneck repetition, shortcut usage, group
        convolutions, and expansion.
        ...
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.Sequential(*(Bottle3neck(c_, c_, r1, r2, r3, r4, shortcut=True, g=1, e=1.0) for _ in range(n)))
